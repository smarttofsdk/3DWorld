    ...
    for i in range(num-3):
        ...
        for i1 in par_line:
            for i2, i3 in itt.combinations(org_line,2):
                ...
                r = calc_4point_area(c0, c1, c3, c2)
...
def get_img_sample(img_gray,s): 
    ...
    return np.array([img_gray[max(0,min(ymax-1,int(round(y)))),max(0,min(xmax-1,int(round(x))))] for x,y in s])
...
def verify_rect_by_edge(img_zero_edge,n0,n1,n2,n3,th=RECT_EDGE_DIST_MAX):
    ...
    pts,num=gen_line_sample(n0,n1); e1=get_img_sample(img_dt,pts); e1=e1.max() if num<RECT_EDGE_CUT*2+1 else e1[RECT_EDGE_CUT:num-RECT_EDGE_CUT].max()
    pts,num=gen_line_sample(n2,n3); e2=get_img_sample(img_dt,pts); e2=e2.max() if num<RECT_EDGE_CUT*2+1 else e2[RECT_EDGE_CUT:num-RECT_EDGE_CUT].max()
    pts,num=gen_line_sample(n0,n2); e3=get_img_sample(img_dt,pts); e3=e3.max() if num<RECT_EDGE_CUT*2+1 else e3[RECT_EDGE_CUT:num-RECT_EDGE_CUT].max()
    pts,num=gen_line_sample(n1,n3); e4=get_img_sample(img_dt,pts); e4=e4.max() if num<RECT_EDGE_CUT*2+1 else e4[RECT_EDGE_CUT:num-RECT_EDGE_CUT].max()
...
def calc_box_dep_amp(img_dep,img_amp,n0,n1,n2,n3,mask=None,erode_ker=KER_ERODE):
    ...
    Confidence = valid_count / img_sel.sum()
    if mask is not None: 
        img_sel[~mask]=0
    ...
    dep=0 if a==0 else img_dep[img_sel>0].sum()/a#np.mean(np.clip(img_dep[img_sel>0],dep_median-0.1,dep_median+0.1))
    amp=0 if a==0 else img_amp[img_sel>0].sum()/a
    return dep,amp,Confidence
...
class speed_tracker:
    def __init__(self, sz=2):
        self.pt_list = [[0,0,0] for n in range(sz)]

    def update(self,data):
        self.pt_list.append(data)
        self.pt_list.pop(0)

    def cal_speed(self,new_data,dep,frame_rate=DMCAM_FRAMERATE,LOST_THRESH=3):# frame_idx u v
        self.update(new_data)
        delta=self.pt_list[-1][0]-self.pt_list[-2][0]
        if delta>LOST_THRESH:
            speed = 0
            return speed
        time=delta*(1.0/frame_rate)
        central_location=pixel_to_pc(np.array(self.pt_list).astype(np.int16)[:,1:3],MEAS_FX, MEAS_FY, MEAS_CX, MEAS_CY,dep=dep)
        dist=calc_point_dist(central_location[0],central_location[1])
        speed=dist/time
        return speed
...
def box_dep_to_hgt(dep,amp):
    if   MEAS_HGT_CORR==1: return MEAS_HGT_PARAM_A* dep    +MEAS_HGT_PARAM_B
    elif MEAS_HGT_CORR==2: return MEAS_HGT_PARAM_A*(dep**2)+MEAS_HGT_PARAM_B*dep+MEAS_HGT_PARAM_C
    elif MEAS_HGT_CORR==3: return MEAS_HGT_PARAM_A* dep    +MEAS_HGT_PARAM_B*amp+MEAS_HGT_PARAM_C
    elif MEAS_HGT_CORR==4: return MEAS_HGT_PARAM_A*(dep**2)+MEAS_HGT_PARAM_B*dep+MEAS_HGT_PARAM_C*amp+MEAS_HGT_PARAM_D
    elif MEAS_HGT_CORR==5: return MEAS_HGT_PARAM_A*(dep**2)+MEAS_HGT_PARAM_B*dep+MEAS_HGT_PARAM_C*(amp**2)+MEAS_HGT_PARAM_D*amp+MEAS_HGT_PARAM_E
...
def box_wid_len_corr(w,l,dep,amp):
    if   MEAS_WID_LEN_CORR==0: return w*dep/MEAS_F, l*dep/MEAS_F
    elif MEAS_WID_LEN_CORR==1: return w*(dep-amp*6e-5)/MEAS_F, l*(dep-amp*6e-5)/MEAS_F
...
def background_det(cam,viewer):
    ...
    if GROUND_CORR:
        ...
        clip_mask=np.bitwise_and(bg_dep>(GROUND_DIST-0.5),bg_dep<(GROUND_DIST+0.5))
...
def convert_to_binary(original_image):
    ...
    gray_image =original_image.clip(0,255).astype(np.uint8).reshape((240,320))#cv2.cvtColor(np.tile(np.expand_dims(original_image,-1),[1,1,3]), cv2.COLOR_RGB2GRAY)
    ...
    while True:
        ...
        if frame_cnt_new%test_frame==0:
            ...
            if frame_cnt_new//(test_frame)==num_test[box_id-1]:
                ...
                test_id=1
                LOG_FNAME = './calib_data/test_' + str(test_id) + ' _box_' + str(box_id) + '.txt'
                log_out =('请准备校准下一个盒子！\n Be ready to test with another box. (20s)')
                print(log_out)
                time.sleep(20)
            else:
        ...
        while go_on:
            ...
            for i in range(len(rect)):
                ...
                if Confidence < MIN_CONFIDENCE:  # 如果矩形内的有效点太少认为结果不可信任
                    break
...
def main():
    ...
    while True:
        ...
        cv2.putText(img_dep_view, '[%d]' % frame_cnt, (2, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        #viewer.update_pan_img_rgb(img_dep_view, pan_id=(0, 0))
        viewer.update_pan_img_gray(img_dep*80, pan_id=(0, 0))
        # 图像预处理
        mask = np.bitwise_and(mask, img_dep < IMG_DEP_SAT_TH)  # 最大深度过滤
        ...
        # 显示图像
        viewer.update_pan_img_rgb(img_dep_cmap, pan_id=(1, 0))
        viewer.update_pan_img_rgb(img_to_cmap(1 - img_edge, mask=None, vmin=0, vmax=1), pan_id=(1, 1))
        if img_amp is not None: viewer.update_pan_img_gray(img_amp * VIEW_AMP_SCALE, pan_id=(0, 1))
