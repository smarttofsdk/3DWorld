...
PLAYBACK_DEP    = './data/dep_1008-0.bin'
PLAYBACK_AMP    = './data/amp_1008-0.bin'
PLAY_TYPE = np.float32
...
####################
# 图像常量
IMG_WID, IMG_HGT =320, 240# 640,480 
...
BACKGROUND_SKIP     = 10    # 背景检测时跳过开始的图像帧数
...
BACKGROUND_MASK[:,195:365]=True

####################
# 地面检测
GROUND_CORR     = False             # 地面倾角修正
GRAND_DEPTH_CALIBRATE_FROM_FILE = True
...
GROUND_MASK = np.zeros((IMG_HGT,IMG_WID),dtype=bool) # 预定义的背景屏蔽区域
#BACKGROUND_MASK[20:IMG_HGT-20,20:IMG_WID-20]=True
GROUND_MASK[:,150:350]=True

GROUND_DIST_MEAS   = False          # 是否测量相机到地面距离
GROUND_DIST        = 1.9 #0.715          # 相机距地面高度(标称值)
...
BG_NOISE_TH        = 0.01          # 背景噪声切割门限，低于他的被认为是噪声

####################
# 镜头参数检测
MEAS_UNDISTORT     = True           # 镜头去畸变
MEAS_DEP_TO_Z      = False           # 射线距离转成Z距离
...
#sony
MEAS_FX            = 524.361022     # 计算x/y尺寸的镜头参数
MEAS_FY            = 525.245972     # 计算x/y尺寸的镜头参数
MEAS_CX            = 311.563996
MEAS_CY            = 255.233994
MEAS_DVEC          = np.array([-0.248168, 0.18924, -0.004633, -0.004769, -0.127264])
