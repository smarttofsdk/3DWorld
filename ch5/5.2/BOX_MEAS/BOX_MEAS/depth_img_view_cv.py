...
img_hgt,img_wid= IMG_HGT,IMG_WID
## 深度图回放相机
class playback_cam_c:
    ...
    def get_dep_amp(self):
        ...
        if self.fp_dep is not None:
            frame_dep=np.fromfile(self.fp_dep,dtype=PLAY_TYPE,count=self.img_sz)
            if (len(frame_dep)<self.img_sz) or (self.frame_cnt>self.rewind):
                self.fp_dep.seek(self.skip_byte,os.SEEK_SET)
                self.frame_cnt+=self.skip
                frame_dep=np.fromfile(self.fp_dep,dtype=PLAY_TYPE,count=self.img_sz)
            frame_dep=frame_dep.astype(np.float32)/1.
        else:
            frame_dep=None
        
        if self.fp_amp is not None:
            frame_amp=np.fromfile(self.fp_amp,dtype=PLAY_TYPE,count=self.img_sz)#float32
            if (len(frame_amp)<self.img_sz) or (self.frame_cnt>self.rewind):
                self.fp_amp.seek(self.skip_byte,os.SEEK_SET)
                if self.fp_dep is None: self.frame_cnt+=self.skip
                frame_amp=np.fromfile(self.fp_amp,dtype=PLAY_TYPE,count=self.img_sz)
            frame_amp=frame_amp.astype(np.float32)
        else:
            frame_amp=None
...
#   flipud/fliplr   图像反转标志
#   cam             象机设备对象
def cal_paras(viewer,cam,
               num=20,\
               row1=0,row2=img_hgt,col1=0,col2=img_wid):
    vaild_img_hgt=row2-row1
    vaild_img_wid=col2-col1
    frames_dep=np.zeros((num, vaild_img_hgt*vaild_img_wid))   # to storge depth image
    overexposure=0  # the rate(number) of overexposured pixels
    variance=0      # the variance of depths of pixels
    noise=np.zeros((1, vaild_img_hgt*vaild_img_wid))
    frame_id=0
    dep_id=-50
    xb_in,xf_in,yb_in,yf_in=col1,col2,row1,row2
    set_area=False
    while True:
        img_dep,img_amp,frame_id_new=cam.get_dep_amp()
        evt,pos=viewer.poll_evt()
        if evt=='md0':
            set_area=True
            xd,yd = pos
            xb_in = xd % 320
            yb_in = yd % 240
            print('begin:(',xb_in,',',yb_in,')')
            while True:
                evt,pos=viewer.poll_evt()
                if evt=='mm':#
                    xm,ym=pos
                    viewer.update_pan_img_rgb(img_to_cmap(img_dep,mask=None,vmin=0,vmax=5).reshape(img_hgt,img_wid,3),pan_id=(0,0))
                    viewer.update_pan_img_gray(img_amp*0.5,pan_id=(0,1))
                    viewer.draw_rectangle(xd,yd,xm,ym,(0,0,255),5)
                elif evt=='mu0':
                    xu,yu=pos
                    xf_in=xu%320
                    yf_in=yu%240
                    print('end:(',xf_in,',',yf_in,')')
                    break
            if xf_in<xb_in: xf_in,xb_in=xb_in,xf_in
            if yf_in<yb_in: yf_in,yb_in=yb_in,yf_in
        # 检查是否有新的数据帧到达
        if frame_id==frame_id_new:
            time.sleep(0.001)
            continue
        ...
        for i in range(row1,row2):
            for j in range(col1,col2):
                if img_dep[i][j]>5 or img_dep[i][j]<0:
                    overexposure+=1
        # 显示图像
        viewer.update_pan_img_rgb(img_to_cmap(img_dep,mask=None,vmin=0,vmax=5).reshape(img_hgt,img_wid,3),pan_id=(0,0))
        viewer.update_pan_img_gray(img_amp*0.5,pan_id=(0,1))
        ...
        viewer.update()
        # 保存数据
        frames_dep[dep_id,:]=img_dep[row1:row2,col1:col2].flatten()
    #calculate the overexposure rate
    overexposure=overexposure/float(num)
    #calculate the noise by variance
    for i in range(vaild_img_hgt*vaild_img_wid):
        temp=frames_dep[:,i]
        temp=np.delete(temp,np.argwhere(temp >10))
        if temp.size<=1: noise[0,i]=64.5
        else: noise[0,i]=np.var(temp)/np.mean(temp)
    meanoise= np.mean(noise)
    #pick only pixels with smaller noise
    noise_tiny=np.delete(noise,np.argwhere(noise >1))
    if noise_tiny.size==0: variance=meanoise
    else: variance=np.mean(noise_tiny)
    #vari=vari.reshape(vaild_img_hgt,vaild_img_wid)
    return overexposure,variance,set_area,xb_in,xf_in,yb_in,yf_in

## 采集多帧dep数据的程序
# 输入
#   num             保存的数据帧数
#   cam             象机设备对象
def record_frame(cam,num=20):
    frames_dep=np.zeros((num, img_hgt*img_wid))   # to storge depth image
    frame_dep=np.zeros((1, img_hgt*img_wid))
    frame_id=0
    dep_id=-50
    while True:
        img_dep,img_amp,frame_id_new=cam.get_dep_amp()
        ...
        if dep_id >= num: break
        # 保存数据
        frames_dep[dep_id,:]=img_dep.flatten()
    
    for i in range(img_hgt*img_wid):
        temp=frames_dep[:,i]
        temp=np.delete(temp,np.argwhere(temp >10))
        if temp.size<1: frame_dep[0,i]=64.5
        else: frame_dep[0,i]=np.mean(temp)
    frame_dep=frame_dep.reshape(img_hgt,img_wid)
    return frame_dep
