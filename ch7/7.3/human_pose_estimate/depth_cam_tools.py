...
class depth_cam_trans_c:
    ...
    def depth_to_pcloud(self, img_dep, valid=[]):
        ...
        if len(valid)>0:
            ...
            pc=np.zeros((sz,3))
            pc[:,0]=(img_dep.ravel())[valid.ravel()]*self.tab_x[valid.ravel()]
            pc[:,1]=(img_dep.ravel())[valid.ravel()]*self.tab_y[valid.ravel()]
            pc[:,2]=(img_dep.ravel())[valid.ravel()]      
        else:
    ...
    def pcloud_to_depth(self,pc,output_idx=False):
        ...
        # img_dep[v_valid, u_valid] = z_valid
...
## 回放保存的深度数据
def play_back_depth_data(fname='depth_frame.bin',img_wid=256,img_hgt=256):
...
class cmap_c:
    ...
    def calc(self,img,valid=[]):
        ...
        if np.size(valid)>0: img_int[~valid.ravel()]=256
...
#     T     ：变换矩阵
def pc_roty(b,pc=[]):    
    T=np.array([[math.cos(b),0,-math.sin(b),0],
                [         0 ,1,          0 ,0],
...
#     如果pc=[]，则返回T，否则返回pc_out
def pc_rotz(b,pc=[]):    
    T=np.array([[ math.cos(b),math.sin(b),0,0],
...
def make_cylinder(L=1.0,R=0.5,KX=80.0,KA=20.0,KS=2.0):
...
def test_cam_save_playback(img_wid=256,img_hgt=256):        
    ...
    play_back_depth_data(img_wid=img_wid,img_hgt=img_hgt)
...
def test_3d(img_wid=256,img_hgt=256):
    fname='./data/left/1709081515_dep.bin'
    fp=open(fname,'rb')
    
    delay=1.0/60.0
    
    viewer=pyg_viewer_c(img_wid,img_hgt)
